...
//						videoheight = "480";
						...
						videoheight = "450";
						try{
							doc.getElementById("embedPopup").hidden = true;
						}catch(e){
							//do nothing
						}
///						testelement = doc.getElementById(videoelement).children[4].children[0];
						testelement = doc.getElementById(videoelement);
						videowidth = testelement.videowidth;
						videoheight = testelement.videoheight;
					}
...
//										}
											...
											if (this.readyState === 4 && this.status === 200) {
												...
												if(replacewebm === true){
													if(delay === 0){
														...
														setTimeout(function () {flvideoreplacerListener.webmReplace(aEvent);}, delay);
													}    
												}else{
													if(delay === 0){
		...
		videoFetch: function(aEvent) {
			...
			var configstr, quality, codecs, timestamp;
...
//				matchpattern = /watch.?-player/.test(doc.getElementById("body"));
				...
				if (testelement !== null) {
					...
					for (var e = cookieMgr.enumerator; e.hasMoreElements();) {
						...
						if(cookie.host == ".youtube.com"){
							if(cookie.name == "VISITOR_INFO1_LIVE"){
								var fvrcookie = cookie.value;
							}	
						}
					}
					*/
					...
					req.open('GET', xmlsource, true); //asynchronous
					req.send(null);
					req.onreadystatechange = function () {
						if (this.readyState == 4 && this.status == 200) {
							...
							for(var i=0; i< newline.length; i++){
								...
								//match patterns
//								matchpattern = /assets/.test(newline[i]);
								matchpattern = /\"url_encoded_fmt_stream_map\": \"/.test(newline[i]);
								...
								if (matchpattern === true) {
									...
									for(var x=0; x< newdata.length; x++){
										...
										datamatchpattern = /url=http/.test(newdata[x]);
										...
										if (datamatchpattern === true) {
											...
											if (newdata[x].match(/itag=5/)) {
												...
												//store download path
												fmt = "5";
												videourl = newdata[x].replace(/.*url=/,"").replace(/\\u0026.*/,"");
												key = newdata[x].replace(/.*sig=/,"").replace(/\\u0026.*/,"");
												videourl = decodeURIComponent(videourl)+"&signature="+key;
									...
									if (replacevideo === true){
										...
										try{
											var fmt5 = this.prefs.getCharPref("downloadersource.youtube."+videoid+".5");
											if(fmt5){
												if(videoquality === "LOW" || videoquality === "MEDIUM" || videoquality === "HIGH" || videoquality === "SUPER"){
										...
										try{
											var fmt18 = this.prefs.getCharPref("downloadersource.youtube."+videoid+".18");
											if(fmt18){
												if(videoquality === "LOW" || videoquality === "MEDIUM" || videoquality === "HIGH" || videoquality === "SUPER"){
													if(mimetype === "autodetect"){
										...
										try{
											var fmt34 = this.prefs.getCharPref("downloadersource.youtube."+videoid+".34");
											if(fmt34){
												if(videoquality === "LOW" || videoquality === "MEDIUM" || videoquality === "HIGH" || videoquality === "SUPER"){
													if(mimetype === "autodetect"){
														newmimetype = "application/x-flv";
										...
										try{
											var fmt35 = this.prefs.getCharPref("downloadersource.youtube."+videoid+".35");
											if(fmt35){
												if(videoquality === "MEDIUM" || videoquality === "HIGH" || videoquality === "SUPER"){
													if(mimetype === "autodetect"){
														newmimetype = "application/x-flv";
													}
													videourl = fmt35;
										...
										try{
											var fmt22 = this.prefs.getCharPref("downloadersource.youtube."+videoid+".22");
											if(fmt22){
												if(videoquality === "HIGH" || videoquality === "SUPER"){
													if(mimetype === "autodetect"){
														newmimetype = "video/mp4";
													}
													videourl = fmt22;
										...
										try{
											var fmt37 = this.prefs.getCharPref("downloadersource.youtube."+videoid+".37");
											if(fmt37){
										...
										try{
											var fmt38 = this.prefs.getCharPref("downloadersource.youtube."+videoid+".38");
											if(fmt38){
												if(videoquality === "SUPER"){
													if(mimetype === "autodetect"){
														newmimetype = "video/mp4";
										...
										this.prefs.setCharPref("video.youtube."+videoid,JSONStrings);
...
//					req.send(null);
			...
			if(sourceurl.match(/vimeo\.com\/\d{1,8}/)){
				...
				if (testelement !== null) {
					...
					//fetch video JSON part
					newline = pagecontent.split(' = {config:')[1].split(',assets:')[0]

					configstr = JSON.parse(decodeURIComponent(newline));
					videoid = configstr["video"]["id"];
					timestamp = configstr["request"]["timestamp"];
...
//					}
					for (var codec=0; codec<codecs.length;codec++){
						if (codecs[codec][0] in configstr["video"]["files"]){
							var video_codec = codecs[codec][0];
							var video_extension = codecs[codec][1];
							replacevideo = true;
							quality = 'sd';
							if(videoquality === "HIGH" || videoquality === "SUPER"){
								for (var j=0; j<configstr["video"]["files"][codecs[codec][0]].length;j++){
									if (configstr["video"]["files"][codecs[codec][0]][j] === 'hd'){
										quality ='hd';
									}
								}
							};
							if(videoquality === "LOW"){
								for (var j=0; j<configstr["video"]["files"][codecs[codec][0]].length;j++){
									if (configstr["video"]["files"][codecs[codec][0]][j] === 'mobile'){
										quality ='mobile';
									}
								}
							}
					...
					videourl = "http://player.vimeo.com/play_redirect?clip_id="+videoid+"&sig="+key+"&time="+timestamp+"&quality="+quality+"&codecs="+video_codec+"&type=moogaloop_local";
					videoheight = String(testelement.children[0].clientHeight);
					videowidth = String(testelement.children[0].clientWidth);
					if(replacevideo === true){
					
						//declare and store file mime
						try{
							mimetype = this.prefs.getCharPref("vimeomimetype");
						}catch(e){
							//do nothing
						}
...
//						videojson.videoheight = pagecontent.split('itemprop="height" content="')[1].split('">')[0];
						...
						videojson.videourl = videourl;
						JSONStrings = JSON.stringify(videojson);
						this.prefs.setCharPref("video.vimeo."+videoid,JSONStrings);
...
//							key = decodeURIComponent(newline[i]).replace(/\t/g,"").replace(/\n/g,"").replace(/ /g,"").replace(/.*key":"/,"").replace(/"\}.*/,"");
					...
					if(replacevideo === true){
						...
						// get image
						pagecontent = doc.getElementsByTagName("link");
						for(var i=0; i< pagecontent.length; i++){

							if (pagecontent[i].rel === "image_src") {
								thumbnail = pagecontent[i].href
							}
						};

						//store download path
						this.prefs.setCharPref("downloadersource.metacafe."+videoid,videourl);
						...
						videojson.videourl = videourl;
						JSONStrings = JSON.stringify(videojson);
						this.prefs.setCharPref("video.metacafe."+videoid,JSONStrings);
...
//							videourl = decodeURIComponent(newline[i]).replace(/.*encodeURIComponent\('/,"").replace(/'.*/,"");
						...
						if (matchpattern === true) {
							var videotitle = decodeURIComponent(newline[i]).replace(/.*'video_title'.*\: "/,"").replace(/".*/,"");
						};
						matchpattern = /'image_url'/.test(newline[i]);
						if (matchpattern === true) {
							thumbnail = decodeURIComponent(newline[i]).replace(/.*'image_url'.*\: '/,"").replace(/'.*/,"");
						};
						matchpattern = /'encrypted'/.test(newline[i]);
						if (matchpattern === true) {
...
//						testelement = doc.getElementById(videoelement);
						...
						// decrypt videourl
						if(encrypted === "true"){
							videourl = Aes.Ctr.decrypt(videourl, videotitle, 256);
						}
						...
						//store download path
						this.prefs.setCharPref("downloadersource.youporn."+videoid,videourl);
...
//						videojson.videoheight = testelement.videoheight;
						...
						videojson.videourl = videourl;
						JSONStrings = JSON.stringify(videojson);
						this.prefs.setCharPref("video.youporn."+videoid,JSONStrings);
			...
			if(sourceurl.match(/pornhub\.com\/view_video.php\?viewkey=/)){
				...
				if (testelement !== null) {
					...
					if(replacevideo === true){
						...
						//store download path
						this.prefs.setCharPref("downloadersource.pornhub."+videoid,videourl);
						...
						videojson.videourl = videourl;
						JSONStrings = JSON.stringify(videojson);
						this.prefs.setCharPref("video.pornhub."+videoid,JSONStrings);
				...
				if (testelement !== null) {
					...
					for(var i=0; i< newline.length; i++){
						...
						if (matchpattern === true) {
							...
							if(prefermp4 === true){
								videourl = newline[i].replace(/.*mp4_url=/,"").replace(/\&.*/g,"");
								videourl = decodeURIComponent(videourl);
								this.prefs.setCharPref("downloadersource.redtube."+videoid+".mp4",videourl);
							}else{
								videourl = newline[i].replace(/.*flv_h264_url=/,"").replace(/\&.*/g,"");
								videourl = decodeURIComponent(videourl);
								this.prefs.setCharPref("downloadersource.redtube."+videoid+".flv",videourl);
							}
							replacevideo = true;
					...
					if(replacevideo === true){
						...
						videojson.videourl = videourl;
						JSONStrings = JSON.stringify(videojson);
						this.prefs.setCharPref("video.redtube."+videoid,JSONStrings);
...
//			if(sourceurl.match(/youtube.*watch.*v\=/) || sourceurl.match(/vimeo\.com/) || sourceurl.match(/metacafe\.com/)){
			...
			var params, videoplayer, webmplayer, flvideoreplacer, childdivs, videodiv, scripts;
			...
			if(sourceurl.match(/youtube.*watch.*v\=/)){
				...
				try{
					webmplayer = doc.getElementById("video-content");
					webmplayer.parentNode.removeChild(webmplayer.firstChild);
				}catch(e){
					//do nothing
				}
...
//			br.setAttribute("valign","middle");
			...
			if(sourceurl.match(/youtube.*watch.*v\=/)){
				...
				try{
					var downloadurl5 = this.prefs.getCharPref("downloadersource.youtube."+videoid+".5");
					if(downloadurl5 !== null){
						var qoption5 = doc.createElement('option');
						qoption5.setAttribute("value","fmt:5|mime:"+newmimetype+"|url:"+downloadurl5);
						qoption5.textContent = "240p [flv]";
						if(downloadurl5 === videourl){
							qoption5.setAttribute("selected","true");
						}
						qoption5 = qselect.appendChild(qoption5);
					}
				}catch(e){
					//do nothing
				}
				try{
					var downloadurl18 = this.prefs.getCharPref("downloadersource.youtube."+videoid+".18");
					if(downloadurl18 !== null){
						var qoption18 = doc.createElement('option');
						qoption18.setAttribute("value","fmt:18|mime:"+newmimetype+"|url:"+downloadurl18);
						qoption18.textContent = "360p [mp4]";
						if(downloadurl18 === videourl){
							qoption18.setAttribute("selected","true");
						}
						qoption18 = qselect.appendChild(qoption18);
					}
				}catch(e){
					//do nothing
				}
				try{
					var downloadurl34 = this.prefs.getCharPref("downloadersource.youtube."+videoid+".34");
					if(downloadurl34 !== null){
						var qoption34 = doc.createElement('option');
						qoption34.setAttribute("value","fmt:34|mime:"+newmimetype+"|url:"+downloadurl34);
						qoption34.textContent = "360p [flv]";
				...
				try{
					var downloadurl35 = this.prefs.getCharPref("downloadersource.youtube."+videoid+".35");
					if(downloadurl35 !== null){
						var qoption35 = doc.createElement('option');
						qoption35.setAttribute("value","fmt:35|mime:"+newmimetype+"|url:"+downloadurl35);
						qoption35.textContent = "480p [flv]";
						if(downloadurl35 === videourl){
							qoption35.setAttribute("selected","true");
						}
						qoption35 = qselect.appendChild(qoption35);
					}
				}catch(e){
					//do nothing
				}
				try{
					var downloadurl22 = this.prefs.getCharPref("downloadersource.youtube."+videoid+".22");
					if(downloadurl22 !== null){
						var qoption22 = doc.createElement('option');
						qoption22.setAttribute("value","fmt:22|mime:"+newmimetype+"|url:"+downloadurl22);
						qoption22.textContent = "720p [mp4]";
						if(downloadurl22 === videourl){
							qoption22.setAttribute("selected","true");
						}
						qoption22 = qselect.appendChild(qoption22);
					}
				}catch(e){
					//do nothing
				}
				try{
					var downloadurl37 = this.prefs.getCharPref("downloadersource.youtube."+videoid+".37");
					if(downloadurl37 !== null){
						var qoption37 = doc.createElement('option');
						qoption37.setAttribute("value","fmt:37|mime:"+newmimetype+"|url:"+downloadurl37);
						qoption37.textContent = "1080p [mp4]";
						if(downloadurl37 === videourl){
							qoption37.setAttribute("selected","true");
						}
						qoption37 = qselect.appendChild(qoption37);
					}
				}catch(e){
					//do nothing
				}
				try{
					var downloadurl38 = this.prefs.getCharPref("downloadersource.youtube."+videoid+".38");
					if(downloadurl38 !== null){
						var qoption38 = doc.createElement('option');
						qoption38.setAttribute("value","fmt:38|mime:"+newmimetype+"|url:"+downloadurl38);
						qoption38.textContent = "Original HD [mp4]";
						if(downloadurl38 === videourl){
							qoption38.setAttribute("selected","true");
						}
						qoption38 = qselect.appendChild(qoption38);
					}
				}catch(e){
					//do nothing
				}
...
//					}
		...
		videoReplace: function(aEvent,aBranch) {
			...
			var ch = ios.newChannel(videourl,null,null);
			var httpChan = ch.QueryInterface(Components.interfaces.nsIHttpChannel);
			httpChan.requestMethod = "HEADER";
			ch.open();
			if (ch.responseStatus === 302) {
				videourl = ch.getResponseHeader("Location");
			};
			...
			if(replacemethod === "embedded"){
				...
				if((newmimetype === "application/x-totem-plugin" && pluginxtp === true)
					...
					newmimetype = flvideoreplacerPlugins.pluginSwitch();
					if(newmimetype !== false){
						plugin = true;
					}
				}
		...
		showHideMenus: function () {//show and hide context menus
			...
			if(enabled === true){
				//declare variables
				var detection = false, pagecontent,videoid, aSite, vidfilename, newvidfilename, downloadurl=null, downloadurl5=null, downloadurl18=null, downloadurl34=null, downloadurl35=null, downloadurl22=null, downloadurl37=null, downloadurl38=null;
		...
		vidDownloader: function (aSite,aFile,aID,aFMT) {
			...
			if(aFMT === "0"){
				sourceurl = this.prefs.getCharPref("downloadersource."+aSite+"."+aID);
			}else{
				sourceurl = this.prefs.getCharPref("downloadersource."+aSite+"."+aID+"."+aFMT);
			}
